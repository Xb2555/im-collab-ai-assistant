...
@Component
public class SceneCExecutionGuard {

    private final ObjectProvider<RedissonClient> redissonClientProvider;
    private final Map<String, ReentrantLock> localLocks = new ConcurrentHashMap<>();

    public SceneCExecutionGuard(ObjectProvider<RedissonClient> redissonClientProvider) {
    ...
    public void execute(String lockKey, Runnable action) {
        ...
        if (redissonClient != null) {
            executeWithDistributedLock(redissonClient.getLock("harness:sceneC:" + lockKey), action);
            return;
        }
