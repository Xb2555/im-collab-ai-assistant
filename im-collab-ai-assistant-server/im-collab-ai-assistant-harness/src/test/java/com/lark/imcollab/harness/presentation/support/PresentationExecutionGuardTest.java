...
class PresentationExecutionGuardTest {
    ...
    void distributedLockWaitsForPreviousAttemptToDrain() throws Exception {
        ...
        when(redissonClient.getLock("harness:presentation:task-1")).thenReturn(lock);
        when(lock.tryLock(5, 600, TimeUnit.SECONDS)).thenReturn(true);
        when(lock.isHeldByCurrentThread()).thenReturn(true);

        new PresentationExecutionGuard(provider).execute("task-1", () -> {
        });
